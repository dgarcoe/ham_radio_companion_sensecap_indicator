...
static pota_row_t s_rows[UI_POTA_ROWS_PER_PAGE];

static int s_page;
static int s_filter_band;
static int s_filter_mode;

EXT_RAM_BSS_ATTR static app_pota_state_t s_snap_buf;
...
/* POTA spots carry mode explicitly. Just bucket it. */
...
static bool spot_passes_filters(const app_pota_spot_t *sp)
{
    if (s_filter_band != 0) {
...
static void build_row(lv_obj_t *parent, pota_row_t *r)
{
    ...
    r->park = lv_label_create(r->row);
    lv_obj_set_style_text_color(r->park, UI_COL_MUTED, 0);
    lv_obj_set_style_text_font(r->park, &lv_font_montserrat_14, 0);
    lv_label_set_long_mode(r->park, LV_LABEL_LONG_CLIP);
    lv_obj_set_width(r->park, LV_PCT(100));
}

/* --- Refresh --- */

static void refresh(const app_pota_state_t *state)
{
    if (!state) return;
    ...
    /* Apply filters to a pointer array, newest first. The POTA backend
     * stores spots in arrival order from the API, which is roughly
     * recent-first already - keep that order. */
    const app_pota_spot_t *filtered[APP_POTA_MAX_SPOTS];
    int filtered_n = 0;
    ...
    int max_page = (total + UI_POTA_ROWS_PER_PAGE - 1) / UI_POTA_ROWS_PER_PAGE - 1;
    ...
    int start = s_page * UI_POTA_ROWS_PER_PAGE;
    ...
    for (int i = 0; i < UI_POTA_ROWS_PER_PAGE; i++) {
        ...
        const app_pota_spot_t *sp = filtered[offset];
        pota_row_t *r = &s_rows[i];
        ...
        if (sp->park_name[0]) {
            snprintf(park, sizeof(park), "%s | %s | %s",
                     sp->park_ref, sp->park_name, mode);
        } else if (sp->park_ref[0]) {
            snprintf(park, sizeof(park), "%s | %s", sp->park_ref, mode);
        } else {
            snprintf(park, sizeof(park), "%s", mode);
        }
        lv_label_set_text(r->park, park);
...
/* --- Widget helpers (compact pager + filter chip, identical to DX) --- */
...
lv_obj_t *ui_pota_create(lv_obj_t *parent, const app_config_t *cfg)
{
    ...
    lv_label_set_text(title, "POTA SPOTS");
    ...
    /* Filter chips - just Band + Mode for POTA (no area/continent here:
     * spots are inherently location-tagged by the park itself). */
    ...
    lv_label_set_text(s_empty_lbl, "Fetching from pota.app...");

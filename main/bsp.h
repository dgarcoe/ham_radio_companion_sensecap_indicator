#pragma once

#include "esp_err.h"
#include "lvgl.h"

/* Initialize display + touch via the Seeed SenseCAP Indicator BSP and bring up LVGL.
 * Returns once LVGL is ticking on its own task. */
esp_err_t bsp_display_start(void);

/* Acquire/release the LVGL mutex around UI calls from app tasks. */
bool bsp_lvgl_lock(int timeout_ms);
void bsp_lvgl_unlock(void);

/* Play the device's "alert" sound. Drives the MLT-8530 piezo via a
 * COBS-encoded BEEP_ON command sent to the auxiliary RP2040 over UART2
 * (TX=GPIO19, RX=GPIO20, 115200). Requires the stock Seeed RP2040
 * firmware. First call lazily brings up the UART. */
void bsp_alert_beep(void);

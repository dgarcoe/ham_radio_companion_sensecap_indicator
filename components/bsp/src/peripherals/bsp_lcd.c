...
/* st7796 / st7789 panel headers omitted - the SenseCAP Indicator only uses
 * ST7701S over RGB. */
#include "sdkconfig.h"
...
#endif
    /* SenseCAP Indicator is always RGB / ST7701; SPI panel drivers were
     * removed when this BSP was vendored. */
    if (LCD_IFACE_RGB != brd->LCD_IFACE) {
        ESP_LOGE(TAG, "Non-RGB LCD interface not supported in this build");
